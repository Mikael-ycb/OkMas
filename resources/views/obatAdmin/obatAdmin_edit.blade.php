<x-layout4>
    <section class="px-20 pt-12 pb-6 bg-white">
        ...
        <h2 class="text-2xl font-semibold text-blue-900 mt-1">Edit Obat</h2>
    </section>

    <form action="{{ route('obatAdmin.update', $obat->id) }}" method="POST" class="px-20 pb-20 bg-white">
        @csrf
        <div class="grid grid-cols-2 gap-6 bg-white p-6 rounded-lg shadow border">
            <div>
                ...
                <input type="text" name="dosis" value="{{ $obat->dosis }}" class="w-full border rounded p-2" required>

                <label class="block font-semibold mt-3 mb-1">Penyakit</label>
                <input type="text" name="penyakit" value="{{ $obat->penyakit }}" class="w-full border rounded p-2">
            </div>

            <div>
                <label class="block font-semibold mb-1">EXP (Bulan/Tahun)</label>
                <input type="text" name="exp" value="{{ $obat->exp }}" class="w-full border rounded p-2" placeholder="MM/YYYY" required>
